...
class AIExecutionRunner(models.AbstractModel):
    ...
    def run(self, profile_code, trigger, session_id=None, entry_prompt=None,
        ...
        # The guard decides the mode, not the runner. A handoff run used to be
        # autonomous "for the same reason a cron one is: there is no human in
        # it" -- which stopped being true when arrival stopped entering the
        # agent by itself. A person starts the work now, so the run carries
        # that person: their ACLs, their eligibility, their company scope.
        execution_mode = security.resolve_mode(trigger)
